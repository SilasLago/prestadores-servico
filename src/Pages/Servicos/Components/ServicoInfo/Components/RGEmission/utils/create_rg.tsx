...
import { bigTitleStyle, blankSpaceStyle, borderBottomStyle, borderLeftStyle, borderRightStyle, borderTopStyle, keyStyle, signatureStyle, titleStyle, valueStyle } from "./create_rg_styles";
...
function exportXLSXWithData(rgData: RG, workbook: XLSX.WorkBook) {
  ...
  workbookKeys.forEach(wkey => {
    ...
    const isFields = finder !== "_" && finder !== "x" && finder !== "z" && finder !== "y" && finder !== "w";
    if (isFields) {
      if(finder === "Coordenador" || finder === "Financeiro" || finder === "Diretoria") {
        if (workbook.Sheets["Prestador - Reembolso"][wkey].s) {
          workbook.Sheets["Prestador - Reembolso"][wkey].s = signatureStyle;
        }
      }else {
      ...
      if (workbook.Sheets["Prestador - Reembolso"][wkey].s) {
        if(finder === "_") {
          ...
          workbook.Sheets["Prestador - Reembolso"][wkey].s = borderRightStyle;
        }else if(finder === "x") {
          workbook.Sheets["Prestador - Reembolso"][wkey].s = borderTopStyle;
        }else if(finder === "y") {
          workbook.Sheets["Prestador - Reembolso"][wkey].s = borderLeftStyle;
        }else if(finder === "w") {
          workbook.Sheets["Prestador - Reembolso"][wkey].s = borderBottomStyle;
        }
      }
